...
$DomainAdminsList = (Get-ADGroupMember -Recursive -Identity "Domain Admins" | Get-ADUser -Properties * | select Name, DistinguishedName, Enabled, whenCreated, whenChanged, LastLogonDate, PasswordLastSet, PasswordNeverExpires, PasswordNotRequired,@{Name="Group Membership"; Expression = {Get-ADPrincipalGroupMembership $_.DistinguishedName | select Name | convertto-csv -NoTypeInformation | select -Skip 1}})
$DomainAdmins = ($DomainAdminsList | Measure-Object).Count

$SchemaAdminsList = (Get-ADGroupMember -Recursive -Identity "Schema Admins" | select Name, DistinguishedName, Enabled, Created, Modified, LastLogonData, PasswordLastSet, PasswordNeverExpires, PasswordNotRequired,@{Name="Group Membership"; Expression = {Get-ADPrincipalGroupMembership $_.DistinguishedName | select Name | convertto-csv -NoTypeInformation | select -Skip 1}})
$SchemaAdmins = ($SchemaAdminsList | Measure-Object).Count

$EnterpriseAdminsList = (Get-ADGroupMember -Recursive -Identity "Enterprise Admins" | select Name, DistinguishedName, Enabled, Created, Modified, LastLogonData, PasswordLastSet, PasswordNeverExpires, PasswordNotRequired,@{Name="Group Membership"; Expression = {Get-ADPrincipalGroupMembership $_.DistinguishedName | select Name | convertto-csv -NoTypeInformation | select -Skip 1}})
